...
class Books extends React.Component {
  ...
  populateBookSearch() {
    //poulates the selection box with books returned from google
  ...
  mykeydown() {
    //need to clear the time out on every key press
    clearTimeout(this.timerID);
  }
  searchResults(e) {
    //searchs google books
    this.setState({
      booksearch: [], //empty out search arr
    });

    let bookSearched = e.target.value;

    if (!bookSearched.length) {
      return;
    } //do nothing for an empty search
    let urlBuild =
      "https://www.googleapis.com/books/v1/volumes?q=" + bookSearched;
    this.timerID = setTimeout(() => {
      //after experimenting google books api erros out on too many
      //requets at once so put on a 1 second timer
      axios
        .get(urlBuild)
        .then((response) => {
          //if succesful parse response from google and set client state
          let parsedData = response.data.items.map((b) => {
            return [b.volumeInfo, b.id];
          });
          this.setState({
            booksearch: parsedData,
            currentSelection: JSON.stringify(parsedData[0]),
          });
        })
        .catch((err) => {
          console.log(err);
  ...
  addBook() {
    //lets client add a book from selection box
    const { currentSelection } = this.state;
    const book = JSON.parse(currentSelection); //note storing value as string other wise can not retireve
    //convert to img link to https to avoid console mixed content warning
    let imglinkHttps =
      book[0].imageLinks.thumbnail.split(":")[0] +
      "s:" +
      book[0].imageLinks.thumbnail.split(":")[1];
    let storeBookInfo = {
      //prepare to send to db
  ...
  swapping(sID) {
    //called on an approval fo a book swap
    this.setState({
      swappingId: sID,
    });
  }
            ...
            <Col xs={12} md={6}>
              ...
              <FormGroup controlId="formBookAdd" type="text">
                ...
                <FormControl
                  ...
                  onKeyDown={this.mykeydown.bind(this)}
                  onKeyUp={this.searchResults.bind(this)}
                />
              </FormGroup>
              ...
              <Trades
                currentUser={this.props.user.user.userName}
                swapped={(s) => this.swapping(s)}
              />
            </Col>
            <Col xs={12} md={6}>
              <div className="text-center">
              ...
              <Bookview
                newBook={this.state.addedBook}
                viewType="user"
                swap={this.state.swappingId}
              />
            </Col>
          </Row>
